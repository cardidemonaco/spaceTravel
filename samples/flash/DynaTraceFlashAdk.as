package 
{
	...
	public class DynaTraceFlashAdk
	{
		...
		{
			ExternalInterface.call("dynaTrace.setAutomaticActionDetection", enabled);
		}

		/**
		 * Indicates the start of onload. Can be called to indicate when onload of a framework starts.
		 */
		public function signalLoadStart(time:Number = -1):void
		{
			ExternalInterface.call("dynaTrace.signalLoadStart", time);
		}

		/**
		 * Indicates that a load end was reached. A framework can have more than one of them. The last one ends the onload action.
		 */
		public function signalLoadEnd(time:Number = -1):void
		{
			ExternalInterface.call("dynaTrace.signalLoadEnd", time);
		}
